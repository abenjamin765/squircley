...
describe("Shape Generation", () => {
  describe("calculateSuperellipsePoint", () => {
    ...
    test("calculates point at angle π/2 (topmost point)", () => {
      ...
      expect(result.y).toBeCloseTo(100, 5); // Positive in SVG coordinate system
    });
    ...
    test("handles different power values", () => {
      const resultPower2 = calculateSuperellipsePoint(Math.PI / 4, 100, 100, 2);
      const resultPower4 = calculateSuperellipsePoint(Math.PI / 4, 100, 100, 4);

      // At 45 degrees, both should be valid points on the superellipse
      expect(Math.abs(resultPower2.x)).toBeGreaterThan(0);
      expect(Math.abs(resultPower2.y)).toBeGreaterThan(0);
      expect(Math.abs(resultPower4.x)).toBeGreaterThan(0);
      expect(Math.abs(resultPower4.y)).toBeGreaterThan(0);
    });
  ...
  describe("generateSquircleSvg", () => {
    ...
    test("includes correct colors", () => {
      ...
      const expectedFillRgb = `rgb(${Math.round(
      ...
      const expectedStrokeRgb = `rgb(${Math.round(
      ...
      expect(svgString).toContain(
        `stroke-width="${DEFAULT_PROPERTIES.strokeWeight}"`
      );
    });
