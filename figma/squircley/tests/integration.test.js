 ...
 */
import { generateSuperellipsePath, generateSquircleSvg } from '../src/shape-utils';
describe('Integration Tests', () => {
    describe('Complete Shape Generation Flow', () => {
        test('generates consistent shapes across different curvatures', () => {
            ...
            const svgStrings = curvatures.map(curvature => generateSquircleSvg(curvature));
        ...
        test('path data contains expected SVG commands', () => {
            ...
            expect(pathData).toMatch(/C\s+\d+\.?\d*\s+\d+\.?\d*\s*,?\s*\d+\.?\d*\s+\d+\.?\d*\s*,?\s*\d+\.?\d*\s+\d+\.?\d*/);
    ...
    describe('Mathematical Consistency', () => {
        test('superellipse formula produces smooth curves', () => {
            ...
            const points = angles.map(angle => calculateSuperellipsePoint(angle, 100, 100, 2));
        ...
        test('power parameter affects shape squareness', () => {
            ...
            // Higher power should make the shape more square-like
            // At 45 degrees, higher power should result in point closer to axis
            const distanceFromAxis2 = Math.abs(pointPower2.x);
            const distanceFromAxis4 = Math.abs(pointPower4.x);
            expect(distanceFromAxis4).toBeLessThan(distanceFromAxis2);
        });
    });
    describe('UI and Backend Consistency', () => {
        test('UI and backend use same shape generation logic', () => {
            // Import both modules
            const { generateSuperellipsePath: uiGeneratePath } = require('../src/ui-controller');
            const { generateSuperellipsePath: backendGeneratePath } = require('../src/shape-utils');
        ...
        test('curvature mapping is consistent', () => {
            ...
            const expectedPowers = [0.5, 3.125, 5.25, 7.375, 10];
...
function calculateSuperellipsePoint(angle, semiMajorAxis, semiMinorAxis, power) {
